...
const BookList = () => {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/books`);
  ...
  const handleDelete = async (bookId) => {
    try {
      const response = await fetch(`${process.env.REACT_APP_HOST}:${process.env.REACT_APP_PORT}/book/${bookId}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        fetchBooks();
